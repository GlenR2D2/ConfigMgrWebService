...
namespace ConfigMgrWebService
{
    ...
    public class ConfigMgrWebService : System.Web.Services.WebService
    {
        ...
        [WebMethod(Description = "Get collection variable for filter collection or all collections")]
        public List<CMCollectionVariables> GetCMCollectionVariableByPath(string secret, CMCollectionType collectiontype, string objectpath = null)
        {
            MethodBase method = MethodBase.GetCurrentMethod();
            MethodBegin(method);
            ...
            if (secret == secretKey)
            {
                ...
                {
                    collectionQuery = String.Format("SELECT * FROM SMS_Collection WHERE CollectionType='{0:D}' AND ObjectPath LIKE '{1}'", collectiontype, objectpath);
                }
        ...
        public List<CMNode> GetCMNodeObjects(string secret, CMNodeObjectType objecttype = CMNodeObjectType.SMS_Collection_Device)
        {
            ...
            if (secret == secretKey)
            {
                ...
                //' folders query
                string deviceFolderNodesQuery = String.Format("Select ContainerNodeID,Name,ParentContainerNodeID,FolderGuid From SMS_ObjectContainerNode Where ObjectType='{0:D}'", objecttype);
                string deviceFolderItemsQuery = String.Format("Select ContainerNodeID,MemberID,MemberGuid,InstanceKey From SMS_ObjectContainerItem Where ObjectType='{0:D}'", objecttype);
